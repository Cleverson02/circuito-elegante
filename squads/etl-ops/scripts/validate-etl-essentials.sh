...
missing_tasks=$(for t in process compile enrich extract-keyframes extract-podcast load-workspace-context; do
...
missing_task_catalog=$(for cfg in "$ROOT"/config.yaml "$ROOT"/squad.yaml; do
  name="$(basename "$cfg")"
